...
function query(result: any) { const q: any = {}; for (const name of ['select','eq','in','order','contains','range','single']) q[name] = jest.fn(() => q); q.then = (resolve: any) => Promise.resolve(result).then(resolve); return q; }
it('returns requested items with recorded partial deliveries scoped to their request', async () => {
  ...
  (supabaseAdmin.from as jest.Mock).mockImplementation((table: string) => query({ data: table === 'customers' ? { id: 'customer', vendor_id: 'vendor' } : table === 'orders' ? [{ id: 'account', total: 30, status: 'received', customer_order_requests: [{ id: 'request', sequence: 1, subtotal: 30 }] }] : table === 'order_items' ? [{ id: 'item', order_id: 'account', order_request_id: 'request', quantity: 3, unit_price: 10, subtotal: 30, products: { name: 'Água' } }] : [{ metadata: { order_item_id: 'item', delivered_quantity: 2 } }], error: null }));
