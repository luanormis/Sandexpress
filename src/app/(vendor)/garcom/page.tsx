...
export default function WaiterServicePage() {
  ...
  async function openAccount(umbrella: Umbrella, name: string, phone: string, partySize: number) {
    ...
    const response = await fetch("/api/vendor/manual-accounts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ vendor_id: vendorId, umbrella_id: umbrella.id, name, phone, party_size: partySize }) });
    const data = await response.json().catch(() => ({}));
...
function SharedPaymentModal({ order, vendorId, onClose, onSubmit }: { order: Order; vendorId: string; onClose: () => void; onSubmit: (order: Order, amount: number, paymentMethod: string, payerName: string, note: string) => Promise<void> }) {
  const [summary, setSummary] = useState<{ party_size: number; total: number; base_total: number; service_fee_amount: number; paid_amount: number; remaining_amount: number; payments: Array<{ id: string; amount: number; payer_name: string; payment_method: string }> } | null>(null);
  const [amount, setAmount] = useState(''); const [payerName, setPayerName] = useState(''); const [method, setMethod] = useState('pix'); const [note, setNote] = useState(''); const [people, setPeople] = useState(1); const [error, setError] = useState(''); const [loading, setLoading] = useState(false);
  useEffect(() => { fetch(`/api/account-payments?vendor_id=${vendorId}&order_id=${order.id}`).then(async response => { const data = await response.json(); if (!response.ok) throw new Error(data.error); return data; }).then(data => { setSummary(data); setPeople(registeredPeople(data.party_size)); setAmount(String(accountShare(data.total, data.remaining_amount, data.party_size))); }).catch(() => setError('Nao foi possivel carregar o saldo.')); }, [vendorId, order.id]);
  const remaining = Number(summary?.remaining_amount ?? order.total); const suggested = accountShare(summary?.total ?? order.total, remaining, people);
